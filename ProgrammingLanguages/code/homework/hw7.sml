...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx,dy,e) =>
	let
	    val v = eval_prog(e, env)
	in
	    case v of
		NoPoints => v
	      | Point (x, y) => Point(x + dx, y + dy)
	      | Line (m, b) => Line(m, b + dy - m * dx)
	      | VerticalLine k => VerticalLine(k + dx)
...
fun preprocess_prog e =
  case e of
      LineSegment (sx1, sy1, sx2, sy2) =>
      if sx1 > sx2
      then
	  let val (x2, y2, x1, y1) = (sx1, sy1, sx2, sy2)
	  in
	  ...
	  let val (x1, y1, x2, y2) = (sx1, sy1, sx2, sy2)
	  in
	      if real_close_point (x1, y1) (x2, y2)
	      then Point(x1, y1)
	      else if real_close(x1, x2) andalso y1 > y2
